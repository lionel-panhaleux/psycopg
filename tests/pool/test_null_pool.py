...
def test_row_factory(dsn):
    with NullConnectionPool(dsn, row_factory=class_row(MyRow), name="null") as p1:
        with p1.connection() as conn1:
            (row1,) = conn1.execute("select 1 as x").fetchall()
    assert_type(p1, NullConnectionPool[psycopg.Connection[MyRow], MyRow])
    ...
    assert_type(p2, NullConnectionPool[psycopg.Connection[TupleRow], TupleRow])
    assert_type(conn2, psycopg.Connection[TupleRow])
...
def test_generic_connection_type(dsn):
    ...
    with NullConnectionPool(
        ...
        (row1,) = conn1.execute("select 1 as x").fetchall()
    assert_type(p1, NullConnectionPool[MyConnection[MyRow], MyRow])
    ...
    with NullConnectionPool(dsn, connection_class=MyConnection[TupleRow]) as p2:
        with p2.connection() as conn2:
            (row2,) = conn2.execute("select 2 as y").fetchall()
    assert_type(p2, NullConnectionPool[MyConnection[TupleRow], TupleRow])
